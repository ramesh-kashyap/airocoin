...
public function BankDetail()
    ...
    public function profile_update(Request $request)
    {
        ...
        try{
            ...
            $id=Auth::user()->id;
        $emailId = $user->email;
        $submittedOtp = $request->code;
        // ✅ Step 2: Check OTP
        $otpRecord = PasswordReset::where('email', $emailId)
            ->where('token', $submittedOtp)
            ->first();
        ...
        //   }
          if (isset($post_array['usdtBep20'])) {
             $update_data['usdtBep20'] = $post_array['usdtBep20'];
             }

            if (isset($post_array['bdcDana'])) {
          $update_data['bdcDana'] = $post_array['bdcDana'];
            }
