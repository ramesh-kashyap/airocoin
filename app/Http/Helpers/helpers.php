                      ...
                      $required_directs = 0;
                       ...
                       if($sp_status=="Active")
                       {
                         if($cnt==1)
                          ...
                          if ($Sposnor_cnt < $required_directs) {
                                $pp = 0; 
                            }
...
$user_mid = $data->id;
      ...
      $cnt = 1;
        ...
        $amount = $amt/100;
              ...
              $Sposnor_cnt = User::where('sponsor',$sponsor)->where('active_status','Active')->count("id");
              $sponsor_pack = Investment::where('user_id',$Sposnor_status?$Sposnor_status->id:0)->where('status','Active')->sum("amount");
              }
              else
              {
                $Sposnor_status =array();
                ...
                if ($sponsor_pack >= 100 && $sponsor_pack <= 999) {
                    $percent = 3; 
                } elseif ($sponsor_pack >= 1000 && $sponsor_pack <= 4999) {
                    $percent = 4; 
                } elseif ($sponsor_pack >= 5000 && $sponsor_pack <= 9999) {
                    $percent = 5; 
                } elseif ($sponsor_pack >= 10000) {
                    $percent = 7; 
                } else {
                    $percent = 0; 
                }
