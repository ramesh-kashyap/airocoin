...
class User extends Authenticatable
{
    ...
    {
    $balance = (Auth::user()->buy_fundAmt->sum('amount')+Auth::user()->getFund())-(Auth::user()->buy_packageAmt());
    return $balance;
    } 
    ...
    {
        $amt= Fundtransfer::where('transfered_id',Auth::user()->id)->sum('netAmt');
        return $amt;
    }
    ...
    {
        $balance = (Auth::user()->users_incomes())- (Auth::user()->withdraw()+Auth::user()->fundTransfer->sum('amount'));
        return $balance;
    } 
    ...
    {
    return  Income::where('user_id',Auth::user()->id)->sum('awallet');
    } 
